...
"""
Object Detection (On Video) From PlasticNet Tensorflow Saved Model
=====================================
"""
import numpy as np
import os
...
# Flag provides name of video we are testing on
flags.DEFINE_string('video_name', None, 'Video name')
# Flag provides minimum confidence threshold
...
def main(argv):
  ...
  score = FLAGS.score
  # Start video capture with given video file
  cap = cv2.VideoCapture(r'videos/' + str(video_name))
  ...
  # Split the video title from the extension type
  video_title, video_ext = os.path.splitext(str(video_name))
  # Save video file renamed to be a MP4 file
  result = cv2.VideoWriter('detections/result_' + video_title + '.mp4', cv2.VideoWriter_fourcc(*'MP4V'),20.0, size)

  while 1:
      # Will loop through frames of given video and run detection tests on each frame, stops if user presses 'q'.
      _,img = cap.read()
